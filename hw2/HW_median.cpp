...
using namespace IP;
using std::vector;
extern void copyRowTo_Buffer(ChannelPtr<uchar>, short*, int, int);
...
HW_median(ImagePtr I1, int sz, ImagePtr I2)
{
	...
	if (sz == 1) {
		...
		short* buffers[11]; // array of sz pointers
		...
		for (int ch = 0; IP_getChannel(I1, ch, p1, type); ch++) {
			...
			for (int i = 0; i < sz / 2; i++) copyRowTo_Buffer(p1, buffers[i], w, sz);

			// continue to rest of buffers, note here we still begin copying from frist row of I1
			for (int i = sz / 2; i < sz; i++) {
				copyRowTo_Buffer(p1, buffers[i], w, sz);
				p1 += w;
			}
			...
			for (int y = 0; y < h; y++) {  // visit each row
				...
				copyRowTo_Buffer(p1, buffers[nextBufferIndex], w, sz);
				if (p1 < endd - w) p1 += w;
			}
		}
